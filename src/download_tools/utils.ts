      ...
      ariaTools.getStatus(dlDetails.gid, (err, message, filename) => {
        if (err) {
          ...
          if (dlDetails.isUploading) {
            resolve({
              message: `Currently uploading <i>${filename}</i> to Google Drive`
            });
          } else {
            resolve({
              message: message,
...
export function generateStatusMessage(totalLength: number, completedLength: number, speed: number, files: any[]): StatusMessage {
  ...
  var message = `Downloading <i>${fileName}</i> \n <code>${progressString}</code> of ${totalLengthStr} done \n Current speed: ${speedStr}ps \n Time left: ${eta}`;
  var status = {
    message: message,
    filename: fileName,
