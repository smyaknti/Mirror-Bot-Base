...
exports.findAriaFilePath = findAriaFilePath;
/**
 ...
 */
function getFileNameFromPath(filePath, inputPath, downloadUri) {
    if (!filePath) {
 ...
 * @returns {string} The name of the file or directory that was downloaded
 */
function getActualDownloadPath(filePath) {
    // +2 because there are two /'s, after ARIA_DOWNLOAD_LOCATION and after the 36 character subdir
    var baseDirLength = constants.ARIA_DOWNLOAD_LOCATION.length + 38;
    var nameEndIndex = filePath.indexOf('/', baseDirLength);
    if (nameEndIndex === -1) {
...
exports.getActualDownloadPath = getActualDownloadPath;
/**
 ...
 */
function getFilenameFromUri(path, uri) {
    if (path) {
...
function isFilenameAllowed(filename) {
    if (!constants.ARIA_FILTERED_FILENAMES)
        return 1;
    if (filename === TYPE_METADATA)
        return -1;
    for (var i = 0; i < constants.ARIA_FILTERED_FILENAMES.length; i++) {
        if (filename.indexOf(constants.ARIA_FILTERED_FILENAMES[i]) > -1)
            return 0;
    }
    return 1;
}
exports.isFilenameAllowed = isFilenameAllowed;
